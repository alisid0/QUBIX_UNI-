...
//
// The whole built site is published: the app at the root and the library under
// /library. It is a preview mirror, built with VITE_WORKSHOP=1 so the asset
// showcases and missions can be looked at, which qubix.university does not do.
//
//   npm run build:pages && node scripts/publish-pages.mjs
...
if (!existsSync('dist/library')) die('dist/library does not exist.', 'Run "npm run build:pages" first.');
