...
<style>
  :global(html),:global(body){overflow:auto;background:#171510}:global(body){position:static}.showcase{min-height:100vh;max-width:none;padding:20px clamp(14px,3vw,38px);background:radial-gradient(circle at 40% 0,#3e3327,#171510 60%);color:white;overflow:auto}header{max-width:1280px;margin:0 auto 16px;display:flex;justify-content:space-between;align-items:end;gap:16px}header p{margin:0 0 4px;color:#bcb19e;font:900 9px var(--qx-font);letter-spacing:.12em}h1{margin:0;font:700 28px Georgia,serif}nav{display:flex;gap:15px}nav a{color:#e2c7b7;font:800 11px var(--qx-font);text-decoration:none;border-bottom:1px solid currentColor;padding-bottom:3px}main{max-width:1280px;margin:auto;display:grid;grid-template-columns:minmax(0,1fr) 340px;gap:16px}.viewport,aside{border:1px solid rgba(255,255,255,.12);border-radius:18px;overflow:hidden}.viewport{position:relative;min-height:clamp(540px,76vh,780px);background:#e7e0d3;touch-action:none}.viewport :global(canvas){display:block;width:100%;height:100%}.viewport>span{position:absolute;inset:0;display:grid;place-items:center;color:#6d6453;font:800 12px var(--qx-font)}aside{padding:24px;background:#f1ede4;color:#25231f}.eyebrow{margin:0 0 6px;color:#8c4c2e;font:900 9px var(--qx-font);letter-spacing:.12em}h2{margin:0;font:700 23px Georgia,serif}aside>p:not(.eyebrow,.note){color:#655d4d;font:600 12px/1.5 var(--qx-font)}code{font:700 11px ui-monospace,monospace;color:#8c4c2e}dl{margin:20px 0;border-block:1px solid #d8d0be;padding:8px 0}dl div{display:flex;justify-content:space-between;gap:10px;padding:7px 0}dt{color:#766d5b;font:700 10px var(--qx-font)}dd{margin:0;text-align:right;font:800 9.5px var(--qx-font);overflow-wrap:anywhere}.controls{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin-bottom:14px}.controls>span{grid-column:1/-1;color:#766d5b;font:900 9px var(--qx-font);text-transform:uppercase;letter-spacing:.08em}.controls button{min-height:42px;border:1px solid #d8d0be;border-radius:10px;background:white;color:#25231f;font:800 10.5px var(--qx-font);text-transform:capitalize;cursor:pointer}.controls button.active{background:#a85a34;border-color:#a85a34;color:white}.controls button:focus-visible{outline:3px solid #a85a34;outline-offset:2px}.note{margin-top:18px;color:#766d5b;font:700 10px var(--qx-font)}@media(max-width:820px){main{grid-template-columns:1fr}.viewport{min-height:520px}}@media(max-width:520px){.showcase{padding:14px 10px}header{align-items:flex-start}nav{flex-direction:column;align-items:flex-end;gap:6px}.viewport{min-height:400px}aside{padding:18px}}
</style>
