...
<section class="mission-shell qx-shell">
  <MissionMasthead eyebrow={`${MISSING_DATA_MISSION.id} · STOCK REVIEW ASSIGNMENT`} title={MISSING_DATA_MISSION.title}
    roomId="stock-room" roomName="Stock Room · Left bay" progress={progress}
    meta={`${missionComplete ? MISSING_DATA_MISSION.cases.length : caseIndex + 1} OF ${MISSING_DATA_MISSION.cases.length} FEED RECORDS`} />

  <main>
    <section class:open={workstationOpen} class="stage-card m1-workarea">
...
<style>
  ...
  .mobile-monitor{min-height:280px;margin:18px;padding:24px;display:grid;place-items:center;align-content:center;gap:12px;border:10px solid #25231f;border-bottom-width:30px;border-radius:14px;background:#bfe8d6;color:#20382e;cursor:pointer}.mobile-monitor .screen-icon{font:900 42px/1 var(--qx-font);letter-spacing:.04em}.mobile-monitor .screen-icon small{font-size:11.5px}.mobile-monitor>b{font:900 17px var(--qx-font)}.mobile-monitor>small{font:700 13px var(--qx-font)}
