...
<section class="game-plan qx-shell" style={`--game:${game.colour}`}>
  <header><div><p>FOUNDATIONAL COMPANION GAME · AUTHORING PLAN · AI_DRAFT</p><h1>{game.title}</h1><span>{game.volume}</span></div><a href={`?mode=game&mission=foundations&role=${role}`}>Return to the book</a></header>
  <main>
    ...
    <aside><div class="section-head"><div><small>BOOK LINK</small><h2>Chapter unlocks</h2></div><span>7 FOUNDATIONS</span></div><ol>{#each game.unlocks as unlock,index}<li><span>{String(index + 1).padStart(2,'0')}</span><b>{unlock}</b><em>{index === 0 ? 'FIRST SCENARIO' : 'LOCKED IN PLAN'}</em></li>{/each}</ol><p>The game grows with the book. It does not introduce a technical idea before the learner has read and practised its foundation.</p></aside>
  </main>
...
<style>
  :global(.qubix-university){height:auto!important;overflow:visible!important}:global(html),:global(body){overflow:auto;background:#171510}:global(body){position:static}.game-plan{min-height:100vh;max-width:none;padding:24px clamp(12px,4vw,50px) 38px;background:radial-gradient(circle at 50% 0,#3b3329,#171510 62%);color:#f1ede4;overflow:auto}header{max-width:1180px;margin:0 auto 20px;display:flex;align-items:end;justify-content:space-between;gap:18px}header p,.section-head small,.brief>small{margin:0 0 6px;color:#e1a582;font:900 8px var(--qx-font);letter-spacing:.12em}header h1{margin:0;font:700 clamp(33px,5vw,55px) Georgia,serif}header span{display:block;margin-top:5px;color:#c1b8a9;font:700 11px var(--qx-font)}header a{color:#e4bba4;font:850 10px var(--qx-font);text-decoration:none;border-bottom:1px solid currentColor;padding-bottom:3px}main{max-width:1180px;margin:auto;display:grid;grid-template-columns:1.25fr .75fr;gap:15px}.brief,.loop,aside{border-radius:16px;background:#f1ede4;color:#241f16}.brief{grid-column:1/-1;padding:24px;display:grid;grid-template-columns:1fr 1.35fr;gap:4px 28px}.brief small,.brief h2{grid-column:1}.brief h2{margin:0;font:700 27px Georgia,serif}.brief>p{grid-column:2;grid-row:1/3;margin:0;color:#5e574a;font:650 13px/1.55 var(--qx-font)}.brief>div{grid-column:1/-1;margin-top:16px;padding:11px 14px;display:flex;align-items:center;justify-content:center;gap:12px;border-radius:9px;background:#fff}.brief div b{color:var(--game);font:900 10px var(--qx-font)}.brief div span{font:700 9px var(--qx-font)}.brief div i{color:var(--game);font-style:normal}.loop,aside{padding:22px}.section-head{display:flex;align-items:end;justify-content:space-between;gap:12px}.section-head h2{margin:0;font:700 24px Georgia,serif}.section-head>span{color:#756d5e;font:850 7px var(--qx-font);letter-spacing:.08em}.loop-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-top:17px}.loop-grid button{min-height:130px;padding:11px;display:flex;flex-direction:column;align-items:flex-start;border:2px solid #ded6c6;border-radius:10px;background:#fff;color:#241f16;text-align:left;cursor:pointer}.loop-grid button>span{display:grid;place-items:center;width:26px;height:26px;border-radius:50%;background:#ece7dc;font:900 8px var(--qx-font)}.loop-grid b{margin-top:13px;font:800 11px var(--qx-font)}.loop-grid small{margin-top:5px;color:#706856;font:650 8px/1.4 var(--qx-font)}.loop-grid button.active{border-color:var(--game)}.loop-grid button.active>span{background:var(--game);color:#fff}.loop-grid button:focus-visible{outline:3px solid var(--game);outline-offset:2px}.loop article{margin-top:12px;padding:15px;border-left:5px solid var(--game);background:#fff}.loop article span{color:var(--game);font:900 7px var(--qx-font)}.loop article h3{margin:5px 0 3px;font:700 20px Georgia,serif}.loop article p{margin:0;color:#625a49;font:650 10px var(--qx-font)}aside ol{list-style:none;margin:16px 0 0;padding:0;border-top:1px solid #d8d0be}aside li{display:grid;grid-template-columns:30px 1fr auto;gap:8px;padding:11px 0;border-bottom:1px solid #e1dacb;align-items:center}aside li>span{color:var(--game);font:900 8px var(--qx-font)}aside li b{font:800 9px var(--qx-font)}aside li em{color:#877b67;font:850 6px var(--qx-font)}aside>p{margin:15px 0 0;color:#625a49;font:650 9px/1.5 var(--qx-font)}footer{max-width:1140px;margin:15px auto 0;padding:13px 20px;display:flex;justify-content:space-between;gap:15px;border-left:5px solid var(--game);background:#26231e;color:#bdb4a5;font:650 8px var(--qx-font)}footer b{color:#f1ede4}@media(max-width:880px){main{grid-template-columns:1fr}.brief{display:block}.brief>p{margin-top:10px}.brief>div{flex-wrap:wrap}.loop-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:560px){.game-plan{padding:15px 9px 28px}header{align-items:flex-start;flex-direction:column}.brief,.loop,aside{padding:16px}.brief>div{align-items:flex-start;flex-direction:column}.brief div i{transform:rotate(90deg);align-self:center}.loop-grid{grid-template-columns:1fr 1fr}.loop-grid button{min-height:145px}footer{flex-direction:column}}
</style>
