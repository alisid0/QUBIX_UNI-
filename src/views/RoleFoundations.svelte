<script>
  const shared = {
    id: 'shared', number: '0', title: 'Shared Foundations', subtitle: 'What every data role should understand before specialising', colour: '#3e9e2a',
...
<section class="library qx-shell" style={`--volume:${volume.colour}`}>
  ...
  <main>
    ...
    <section class="contents"><div class="contents-head"><div><p>TABLE OF CONTENTS</p><h2>{volume.title}</h2></div><div class="contents-actions"><span>{volume.chapters.length} CHAPTERS · FOUNDATION ONLY</span>{#if volume.bookHref}<a href={volume.bookHref}>Begin Part One · 3 h 55 min →</a>{/if}</div></div>
      <ol>{#each volume.chapters as chapter,index}<li><span>{String(index + 1).padStart(2,'0')}</span><div><h3>{chapter[0]}</h3><p>{chapter[1]}</p></div><em>READ → EXAMPLE → CHECK</em></li>{/each}</ol>
    </section>
  </main>
...
<style>
  :global(.qubix-university){height:auto!important;overflow:visible!important}:global(html),:global(body){overflow:auto;background:#f1ede4}:global(body){position:static}.library{min-height:100vh;max-width:none;padding:25px clamp(12px,4vw,52px) 40px;background:#f1ede4;color:#241f16;overflow:auto}header{max-width:1220px;margin:0 auto 22px;display:flex;align-items:end;justify-content:space-between;gap:20px;border-bottom:1px solid #d8d0be;padding-bottom:19px}header p,.contents-head p{margin:0 0 6px;color:#8c4c2e;font:900 9px var(--qx-font);letter-spacing:.13em}header h1{margin:0;font:700 clamp(31px,5vw,54px) Georgia,serif}header span{display:block;margin-top:8px;color:#625a49;font:650 12px var(--qx-font)}header a{color:#8c4c2e;font:850 10px var(--qx-font);text-decoration:none;border-bottom:1px solid currentColor;padding-bottom:3px;white-space:nowrap}.principle{max-width:1180px;margin:0 auto 16px;padding:14px 20px;display:flex;align-items:center;justify-content:center;gap:13px;border-radius:12px;background:#241f16;color:#f1ede4;font:750 10px var(--qx-font)}.principle b{color:#e4a17b;letter-spacing:.1em}.principle i{color:#83bc68;font-style:normal}nav{max-width:1180px;margin:0 auto 18px;display:grid;grid-template-columns:repeat(5,1fr);gap:8px}nav button{min-height:65px;padding:9px 11px;display:flex;align-items:center;gap:9px;border:1px solid #d8d0be;border-radius:11px;background:#fff;color:#241f16;text-align:left;cursor:pointer}nav button>span{display:grid;place-items:center;flex:0 0 32px;height:32px;border-radius:7px;background:#ece7dc;color:#625a49;font:900 9px var(--qx-font)}nav button div{display:grid;gap:2px}nav b{font:850 9.5px var(--qx-font)}nav small{color:#756c5c;font:650 7.5px var(--qx-font)}nav button.active{border:2px solid var(--volume);background:#fbf8f1}nav button.active>span{background:var(--volume);color:#fff}nav button:focus-visible{outline:3px solid var(--volume);outline-offset:2px}main{max-width:1180px;margin:auto;display:grid;grid-template-columns:300px 1fr;gap:16px;align-items:start}main>aside,.contents{border:1px solid #d8d0be;border-radius:16px;background:#fff;overflow:hidden}main>aside{position:sticky;top:14px;padding:20px}.book{position:relative;min-height:310px;padding:25px;display:flex;flex-direction:column;justify-content:space-between;border-radius:5px 15px 15px 5px;background:var(--volume);color:#fff;box-shadow:inset 8px 0 rgba(0,0,0,.13),6px 8px 0 #ded5c5}.book:after{content:'';position:absolute;left:22px;right:18px;top:60px;height:1px;background:rgba(255,255,255,.4)}.book span{font:900 8px var(--qx-font);letter-spacing:.13em}.book b{max-width:180px;font:700 29px/1.05 Georgia,serif}.book i{align-self:flex-end;font:normal 45px Georgia,serif}aside>p{margin:22px 0 9px;font:700 14px/1.45 Georgia,serif}aside>strong{color:#625a49;font:650 10px/1.55 var(--qx-font)}.requires{margin-top:16px;padding:11px;border-left:4px solid var(--volume);background:#f1ede4}.requires b,.requires span{display:block}.requires b{font:900 8px var(--qx-font)}.requires span{margin-top:3px;font:700 9px var(--qx-font)}.contents{padding:22px}.contents-head{display:flex;align-items:end;justify-content:space-between;gap:12px;padding-bottom:16px;border-bottom:1px solid #d8d0be}.contents-head h2{margin:0;font:700 27px Georgia,serif}.contents ol{list-style:none;margin:0;padding:0}.contents li{display:grid;grid-template-columns:42px 1fr auto;align-items:center;gap:13px;padding:17px 4px;border-bottom:1px solid #e4ddce}.contents li>span{color:var(--volume);font:900 11px var(--qx-font)}.contents h3{margin:0 0 4px;font:700 17px Georgia,serif}.contents li p{margin:0;color:#625a49;font:650 9.5px/1.45 var(--qx-font)}.contents em{color:#8c4c2e;font:850 6.5px var(--qx-font);letter-spacing:.07em;white-space:nowrap}.page-rule{max-width:1140px;margin:17px auto 0;padding:16px 20px;display:flex;align-items:center;justify-content:center;gap:10px;border-radius:13px;background:#e7efdc}.page-rule>p{margin:0 15px 0 0;font:700 11px Georgia,serif}.page-rule div{display:flex;align-items:center;gap:6px}.page-rule div span{display:grid;place-items:center;width:23px;height:23px;border-radius:50%;background:#3e9e2a;color:#fff;font:900 8px var(--qx-font)}.page-rule div b{font:800 8px var(--qx-font)}.page-rule>i{color:#759166;font-style:normal}footer{max-width:1180px;margin:15px auto 0;display:flex;justify-content:space-between;gap:15px;color:#746c5e;font:650 8px var(--qx-font)}@media(max-width:900px){nav{grid-template-columns:repeat(3,1fr)}main{grid-template-columns:1fr}main>aside{position:static;display:grid;grid-template-columns:230px 1fr;gap:0 20px}.book{grid-row:1/5;min-height:270px}.page-rule{flex-wrap:wrap}}@media(max-width:600px){.library{padding:15px 9px 28px}header{align-items:flex-start;flex-direction:column}.principle{align-items:flex-start;flex-direction:column}.principle i{transform:rotate(90deg);align-self:center}nav{display:flex;overflow-x:auto;padding-bottom:4px}nav button{flex:0 0 145px}.contents{padding:15px}.contents-head{align-items:flex-start;flex-direction:column}.contents li{grid-template-columns:32px 1fr}.contents em{grid-column:2}.book{min-height:230px}main>aside{display:block}.book{margin-bottom:18px}.page-rule{align-items:flex-start;justify-content:flex-start;flex-direction:column}.page-rule>i{transform:rotate(90deg);align-self:center}footer{flex-direction:column}}
  .companion{margin-top:16px;padding:14px;border-radius:11px;background:#241f16;color:#f1ede4}.companion small{color:#d9a07f;font:900 7px var(--qx-font);letter-spacing:.1em}.companion h3{margin:6px 0 5px;font:700 17px Georgia,serif}.companion p{margin:0;color:#c8c0b2;font:650 8.5px/1.45 var(--qx-font)}.companion a{display:grid;place-items:center;min-height:38px;margin-top:10px;border-radius:8px;background:var(--volume);color:#fff;font:900 8.5px var(--qx-font);text-decoration:none}.companion a:focus-visible{outline:3px solid var(--volume);outline-offset:2px}
  .contents-actions{display:grid;justify-items:end;gap:8px}.contents-actions>span{color:#756c5c;font:850 7px var(--qx-font);letter-spacing:.08em}.contents-actions>a{padding:9px 11px;border-radius:7px;background:var(--volume);color:#fff;font:900 8px var(--qx-font);text-decoration:none}
</style>
