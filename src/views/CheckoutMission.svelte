<script>
  ...
  function resetMission() {
    ...
    lines = [];
    latestOutcome = null;
    answers = { scan: '', price: '', total: '' };
    missionComplete = false;
    classificationChecked = false;
    feedback = 'Set the requested quantity, then scan the product.';
    feedbackTone = '';
    setTimeout(placeCurrentProduct, 0);
  }
    ...
    <aside>
      ...
      {#if lines.length === CHECKOUT_MISSION.order.length}
        <section class="classify-card">
          ...
          {#if missionComplete}<p class="complete">✓ {CHECKOUT_MISSION.competency}</p><button class="reset" on:click={resetMission}>Run mission again</button>{/if}
        </section>
      {/if}
    </aside>
  </main>
...
<style>
  ...
  header{max-width:1380px;margin:0 auto 14px;display:flex;align-items:center;justify-content:space-between;gap:18px}.identity{display:flex;align-items:center;gap:13px}.role{display:grid;place-items:center;width:48px;height:48px;border-radius:13px;background:#a85a34;color:#fff;font:900 11px var(--qx-font);text-align:center}.identity p{margin:0 0 3px;color:#bcb19e;font:800 9px var(--qx-font);letter-spacing:.1em}.identity h1{margin:0;color:#fff;font:700 27px Georgia,serif}nav{display:flex;gap:15px}nav a{color:#e2c7b7;font:800 12px var(--qx-font);text-decoration:none;border-bottom:1px solid currentColor;padding-bottom:3px}.progress{max-width:1380px;height:5px;margin:0 auto 16px;border-radius:9px;background:rgba(255,255,255,.1);overflow:hidden}.progress span{display:block;height:100%;background:#6bc93f;transition:width .4s ease}
  main{max-width:1380px;margin:0 auto;display:grid;grid-template-columns:minmax(0,1fr) 370px;gap:16px;align-items:start}.stage-card,aside section{border:1px solid rgba(255,255,255,.12);border-radius:18px;background:#f1ede4;color:#241f16;overflow:hidden}.brief{min-height:86px;padding:16px 19px;display:flex;align-items:center;justify-content:space-between;gap:16px}.brief h2,.card-title h2,.classify-card h2{margin:2px 0 3px;font:700 22px Georgia,serif}.brief p:not(.eyebrow){margin:0;color:#726a58;font:700 12px var(--qx-font)}.brief>strong{font:800 28px Georgia,serif;color:#8c4c2e}.eyebrow{margin:0 0 5px;color:#8c4c2e;font:900 9.5px var(--qx-font);letter-spacing:.12em}.viewport{position:relative;min-height:clamp(390px,58vh,650px);background:#efe9dd;border-block:1px solid #d8d0be;overflow:hidden;touch-action:none}.viewport :global(canvas){display:block;width:100%;height:100%}.loading{position:absolute;inset:0;display:grid;place-items:center;padding:28px;color:#726a58;font:800 13px var(--qx-font)}.checkout-controls{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:14px 17px}.quantity{display:flex;align-items:center;gap:9px}.quantity span{margin-right:3px;color:#726a58;font:800 11px var(--qx-font)}button,select{min-height:44px;border:1px solid #d8d0be;border-radius:11px;background:#fff;color:#241f16;font:800 12px var(--qx-font)}button{padding:9px 14px;cursor:pointer}.quantity button{width:44px;padding:0;font-size:18px}.quantity strong{min-width:20px;text-align:center;font:900 15px var(--qx-font)}button:focus-visible,select:focus-visible{outline:3px solid #a85a34;outline-offset:2px}button:disabled{opacity:.45;cursor:not-allowed}.scan{min-width:160px;background:#a85a34;border-color:#a85a34;color:#fff}.feedback{margin:0;padding:12px 17px;border-top:1px solid #d8d0be;color:#726a58;background:#fbf8f1;font:700 12.5px/1.45 var(--qx-font)}.feedback.wrong{color:#a02d1d;background:#f7dcd6}.feedback.right{color:#3c6427;background:#e7efdc}.feedback.working{color:#8c4c2e;background:#f2e4da}
  aside{display:flex;flex-direction:column;gap:12px}aside section{padding:17px;overflow:visible}.lineage{display:grid;gap:8px;margin-top:10px}.lineage>div{display:flex;align-items:center;gap:10px;padding:9px;border:1px solid #e4ddce;border-radius:10px;background:#fbf8f1;opacity:.58}.lineage>div.active{opacity:1;border-color:#a8c797;background:#eff4e8}.lineage span{display:grid;place-items:center;width:25px;height:25px;border-radius:50%;background:#241f16;color:#fff;font:900 10px var(--qx-font)}.lineage p{margin:0;font:600 11px/1.3 var(--qx-font)}.lineage b{display:block;font-size:10px;text-transform:uppercase;letter-spacing:.06em}dl{display:grid;gap:7px;margin:12px 0 0;padding-top:11px;border-top:1px solid #e4ddce}dl div{display:flex;justify-content:space-between;gap:10px}dt{color:#726a58;font:700 10px var(--qx-font)}dd{margin:0;font:800 10.5px var(--qx-font);overflow-wrap:anywhere}.muted{margin:11px 0 0;color:#726a58;font:600 12px/1.45 var(--qx-font)}.card-title{display:flex;align-items:center;justify-content:space-between;gap:12px}.card-title>strong{color:#3c6427;font:800 24px Georgia,serif}.basket-card ol{list-style:none;margin:10px 0 0;padding:0}.basket-card li{display:flex;justify-content:space-between;gap:10px;padding:8px 0;border-top:1px solid #e4ddce;font:700 11px var(--qx-font)}.classify-card label{display:grid;grid-template-columns:1fr 145px;gap:10px;align-items:center;margin-top:9px;color:#57503f;font:700 11.5px var(--qx-font)}select{width:100%;padding:0 9px}.check{width:100%;margin-top:13px;background:#241f16;color:#fff;border-color:#241f16}.complete{margin:12px 0 0;padding:10px;border-radius:9px;background:#e7efdc;color:#3c6427;font:800 11.5px/1.4 var(--qx-font)}.reset{width:100%;margin-top:8px;background:#fff}
