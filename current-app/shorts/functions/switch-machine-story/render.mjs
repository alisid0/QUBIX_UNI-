...
const storyPath = '/shorts/functions/switch-machine-story/';
const outputDir = path.resolve('shorts/functions/switch-machine-story');
const port = 4184;
const mimeTypes = { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.webm': 'video/webm' };
...
const server = http.createServer(async (request, response) => {
  try {
    ...
    response.writeHead(200, { 'Content-Type': mimeTypes[path.extname(filename)] || 'application/octet-stream' }); response.end(data);
...
try {
  ...
  console.log('Rendering Qubix Switch and Machine Short...');
  const downloadReady = page.waitForEvent('download', { timeout: 120000 });
  const result = await page.evaluate(() => window.renderFilm());
  const download = await downloadReady;
  const output = path.join(outputDir, 'qubix-switch-machine-function-short.webm');
