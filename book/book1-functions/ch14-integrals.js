...
export default {
  ...
  practice: [
    { q: 'A car travels at a steady 40 mph for 3 hours. Find the distance as an area.', level: 'Analyse change',
      ...
      show: { kind: 'graph', f: t => 40, title: 'A RECTANGLE OF DISTANCE', note: '40 mph for 3 hours', x0: 0, x1: 3.6, y0: 0, y1: 55, w: 285, h: 200, marks: [[0, 40], [3, 40]],
        caption: 'Speed up, time across, so the region beneath is 40 tall and 3 wide. Its area carries the units miles.' } },
    { q: 'Speed is v(t) = 2t from t = 0 to t = 5. Find the distance using the area of a triangle.', level: 'Analyse change',
      ...
      show: { kind: 'graph', f: t => 2 * t, title: 'A TRIANGLE OF DISTANCE', note: 'v(t)=2t to t=5', x0: 0, x1: 5.6, y0: 0, y1: 12, w: 285, h: 205, marks: [[5, 10], [0, 0]],
        caption: 'Base 5 and height 10 give an area of 25 metres. The speed was never constant, so no single multiplication would have done.' } },
    { q: 'Check question 2 using an antiderivative of 2t.', level: 'Calculate',
      ...
      show: { kind: 'graph', f: t => t * t, title: 'THE ANTIDERIVATIVE', note: 't^2 at the two ends', x0: 0, x1: 5.6, y0: 0, y1: 30, w: 285, h: 205, marks: [[5, 25], [0, 0]],
        caption: 'The same 25, reached without any geometry: evaluate t\u00b2 at both ends and subtract. Two unrelated procedures, one answer.' } },
    { q: 'Which rule has 3x^2 as its derivative? Use it to find the area under 3x^2 from 0 to 2.', level: 'Calculate',
      ...
      show: { kind: 'riemann', f: x => 3 * x * x, n: 12, x0: 0, x1: 2, side: 'right', title: 'AREA UNDER 3x^2', w: 285, h: 205,
        caption: 'The strips close on 8, which is what x\u00b3 gives when evaluated at 2 and 0 and subtracted. Twelve strips already get within half a unit.' } },
    { q: 'Why do the rectangle totals in the table all overestimate the true answer?', level: 'Recognise',
      ...
      show: { kind: 'riemann', f: x => x * x, n: 4, x0: 0, x1: 1, side: 'right', title: 'EVERY TOP OVERSHOOTS', w: 285, h: 205,
        caption: 'Each rectangle takes its height from the right of its strip, and the curve is rising, so every top sits above the curve across the whole width.' } },
    { q: 'Estimate the area under f(x) = x^2 from 0 to 1 with two strips, using left-hand heights instead.', level: 'Calculate', hard: true,
      ...
      show: { kind: 'frames', label: 'Trap the answer between them',
        frames: [
          { kind: 'riemann', f: x => x * x, n: 2, x0: 0, x1: 1, side: 'left', title: 'LEFT: 0.125', w: 260, h: 195, pick: 'left',
            say: 'Every rectangle sits under the curve, so this is too small.' },
          { kind: 'riemann', f: x => x * x, n: 2, x0: 0, x1: 1, side: 'right', title: 'RIGHT: 0.625', w: 260, h: 195, pick: 'right',
            say: 'Every rectangle pokes above it, so this is too big. The true value of one third is trapped between the two.' }
        ] } },
    { q: 'A tank fills at a constant 3 litres per minute for 10 minutes. Sketch the rate against time and find the volume.', level: 'Analyse change',
      ...
      show: { kind: 'graph', f: t => 3, title: 'A CONSTANT RATE', note: '3 litres a minute', x0: 0, x1: 11, y0: 0, y1: 5, w: 285, h: 195, marks: [[0, 3], [10, 3]],
        caption: 'A horizontal line at height 3. The region beneath is 3 tall and 10 wide, so the volume is 30 litres.' } },
    { q: 'Explain in one sentence what the Fundamental Theorem connects.', level: 'Recognise',
      ...
      show: { kind: 'frames', label: 'Two operations, one answer',
        frames: [
          { kind: 'graph', f: t => 2 * t, title: 'ACCUMULATE THE RATE', note: 'area to t=3', x0: 0, x1: 3.6, y0: 0, y1: 7, w: 265, h: 195, marks: [[3, 6]], pick: 'integrate',
            say: 'The triangle beneath v(t) = 2t up to 3 has area 9.' },
          { kind: 'graph', f: t => t * t, title: 'UNDO THE DERIVATIVE', note: 't^2 at 3 and 0', x0: 0, x1: 3.6, y0: 0, y1: 11, w: 265, h: 195, marks: [[3, 9], [0, 0]], pick: 'antidifferentiate',
            say: 'The rule whose derivative is 2t, evaluated at both ends: 9 again. That agreement is the Fundamental Theorem.' }
        ] } },
    { q: 'Find the area under f(x) = x from 0 to 4, first by geometry and then by antiderivative.', level: 'Calculate', hard: true,
      ...
      show: { kind: 'graph', f: x => x, title: 'A TRIANGLE, AND x^2/2', note: 'area 8 either way', x0: 0, x1: 4.6, y0: 0, y1: 6, w: 285, h: 205, marks: [[4, 4], [0, 0]],
        caption: 'Base 4 and height 4 give 8 by geometry. The antiderivative x\u00b2/2 gives 16/2 = 8. Neither route knew about the other.' } },
    { q: 'A speed of −5 metres per second is recorded for 2 seconds. What does the "area" come to, and what does the sign mean?', level: 'Analyse change', hard: true,
      ...
      show: { kind: 'graph', f: t => -5, title: 'BELOW THE AXIS', note: '-5 m/s for 2 s', x0: 0, x1: 2.6, y0: -8, y1: 3, w: 285, h: 205, marks: [[0, -5], [2, -5]],
        caption: 'The region lies under the axis and counts as \u221210. The object moved 10 metres the other way, so the integral reports displacement rather than distance travelled.' } },
    { q: 'Why does making the strips thinner reduce the error, rather than just changing it?', level: 'Recognise', hard: true,
      ...
      show: { kind: 'frames', label: 'Thinner strips, smaller slivers', control: 'slider',
        frames: [
          { kind: 'riemann', f: x => x * x, n: 2, x0: 0, x1: 1, side: 'right', title: '2 STRIPS', w: 255, h: 190, pick: '2',
            say: 'Two large slivers between the tops and the curve.' },
          { kind: 'riemann', f: x => x * x, n: 4, x0: 0, x1: 1, side: 'right', title: '4 STRIPS', w: 255, h: 190, pick: '4',
            say: 'Twice as many slivers, each much less than half the size.' },
          { kind: 'riemann', f: x => x * x, n: 12, x0: 0, x1: 1, side: 'right', title: '12 STRIPS', w: 255, h: 190, pick: '12',
            say: 'Over a narrower strip the curve has less room to depart from the height chosen. The count grows, and each sliver shrinks faster.' },
    ...
    { q: 'Chapter 13 found the derivative of x^3/3. What is it, and why does that make example 2 work?', level: 'Analyse change', hard: true,
      ...
      show: { kind: 'frames', label: 'Down, then back up',
        frames: [
          { kind: 'graph', f: x => x * x * x / 3, title: 'THE ANTIDERIVATIVE', note: 'x^3/3', x0: -0.5, x1: 2, y0: -0.5, y1: 3, w: 265, h: 195, marks: [[1, 1 / 3]], pick: 'x^3/3',
            say: 'Its derivative is x\u00b2, by the same method that gave 3x\u00b2 for x\u00b3 and then dividing by three.' },
          { kind: 'riemann', f: x => x * x, n: 12, x0: 0, x1: 1, side: 'right', title: 'THE AREA IT MEASURES', w: 265, h: 195, pick: 'the area',
            say: 'Finding an area was converted into finding a rule whose derivative is the curve. That conversion is what the Fundamental Theorem licenses.' }
        ] } },
  ],
