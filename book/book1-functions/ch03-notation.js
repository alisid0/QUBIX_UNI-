...
export default {
  ...
  practice: [
    { q: 'For g(x) = x^2 − 3, find g(0), g(2), and g(−2).', level: 'Calculate',
      a: 'g(0) = −3, g(2) = 1, g(−2) = 1.',
      show: { kind: 'frames', label: 'Square the blank, then subtract 3',
        frames: [
          { kind: 'machine', rule: 'x^2 - 3', input: '0', output: '-3', pick: 'g(0)', say: '0 squared is 0; 0 − 3 = −3.' },
          { kind: 'machine', rule: 'x^2 - 3', input: '2', output: '1', pick: 'g(2)', say: '2 squared is 4; 4 − 3 = 1.' },
          { kind: 'machine', rule: 'x^2 - 3', input: '-2', output: '1', pick: 'g(-2)', say: 'The brackets square the minus too, so (−2)² = 4 and the answer is 1 again.' }
        ] } },
    { q: 'For h(t) = 5 − 2t, solve h(t) = −1.', level: 'Calculate',
      a: 't = 3, since 5 − 2t = −1 gives −2t = −6.',
      show: { kind: 'machine', rule: '5 - 2t', input: '3', output: '-1', inLabel: 'what we solved for', outLabel: 'what we were given',
        caption: 'Solving runs the arrow backwards. Checking it forwards costs one line: 5 − 2(3) = −1.' } },
    { q: 'Write a formula for "triple the input, then subtract 4".', level: 'Represent',
      a: 'f(x) = 3x − 4.',
      show: { kind: 'blanks', rule: 'triple the ⬚, then subtract 4', sub: 'the words, with a blank where the input goes', result: 'f(x) = 3x - 4',
        caption: 'Words to formula. The order of the two instructions is what puts the 4 outside the multiplication.' } },
    { q: 'For f(x) = 2x + 1, find f(0), f(−3), and f(0.5).', level: 'Calculate',
      a: '1, −5, and 2.',
      show: { kind: 'graph', f: x => 2 * x + 1, title: 'THREE EVALUATIONS', note: 'f(x)=2x+1', x0: -4, x1: 3, y0: -6, y1: 5, w: 275, h: 200,
        marks: [[0, 1], [-3, -5], [0.5, 2]],
        caption: 'Evaluating means going up from a point on the horizontal axis and reading the height. All three marked points sit on one line.' } },
    { q: 'For f(x) = 2x + 1, solve f(x) = 0.', level: 'Calculate',
      a: '2x + 1 = 0 gives x = −1/2.',
      show: { kind: 'graph', f: x => 2 * x + 1, title: 'SOLVING, ON THE GRAPH', note: 'f(x)=0 at x=-0.5', x0: -3, x1: 3, y0: -5, y1: 6, w: 275, h: 200,
        marks: [[-0.5, 0]],
        caption: 'Solving means starting from a height and reading across to find the input. Here the height is 0, so the answer is where the line crosses the horizontal axis.' } },
    { q: 'Explain in one sentence why f(x) does not mean f times x.', level: 'Recognise',
      ...
      show: { kind: 'blanks', rule: 'f is the name of a rule, not a quantity', sub: 'so there is nothing to multiply by', result: 'f(x) = the output f gives to x',
        caption: 'The brackets say "apply", exactly as they do in sqrt(x) or sin(x). Nobody reads sqrt(9) as sqrt times 9.' } },
    { q: 'For p(x) = 4 − x^2, find p(1), p(−1), and p(3).', level: 'Calculate',
      a: '3, 3, and −5.',
      show: { kind: 'graph', f: x => 4 - x * x, title: 'A FLIPPED PARABOLA', note: 'p(x)=4-x^2', x0: -4, x1: 4, y0: -6, y1: 6, w: 275, h: 205,
        marks: [[1, 3], [-1, 3], [3, -5]],
        caption: '1 and −1 land at the same height, because squaring destroys the sign before the subtraction happens. At 3 the answer has gone below the axis.' } },
    { q: 'For f(x) = 3x − 4, write f(a), f(2a), and f(x + 1) in simplest form.', level: 'Represent',
      a: 'f(a) = 3a − 4; f(2a) = 6a − 4; f(x + 1) = 3(x + 1) − 4 = 3x − 1.',
      show: { kind: 'frames', label: 'Whatever arrives goes in every blank',
        frames: [
          { kind: 'blanks', rule: 'f(⬚) = 3⬚ - 4', sub: 'the input is a', result: 'f(a) = 3a - 4', pick: 'f(a)',
            say: 'A bare letter behaves exactly like a number.' },
          { kind: 'blanks', rule: 'f(⬚) = 3⬚ - 4', sub: 'the input is 2a', result: 'f(2a) = 6a - 4', pick: 'f(2a)',
            say: 'Only the 3a part doubled. The −4 was never multiplied by the input.' },
          { kind: 'blanks', rule: 'f(⬚) = 3⬚ - 4', sub: 'the input is x + 1', result: 'f(x+1) = 3(x+1) - 4 = 3x - 1', pick: 'f(x+1)',
            say: 'The bracket holds the whole input together while it is tripled.' }
        ] } },
    ...
    { q: 'For f(x) = 5x, show that f(x + h) − f(x) does not depend on x.', level: 'Calculate', hard: true,
      ...
      show: { kind: 'steps', f: x => 5 * x, from: 0, to: 4, title: 'THE SAME RISE EVERYWHERE', note: 'f(x)=5x', x0: -0.6, x1: 5.2, y0: -1, y1: 24, w: 285, h: 210,
        caption: 'Each unit step raises the output by 5, wherever you start. That is the algebra of 5h drawn as a staircase, and chapter 6 calls it the linear family.' } },
    { q: 'A table gives f(1) = 4, f(2) = 7, f(3) = 10. Write a formula that fits, and say what it assumes.', level: 'Represent', hard: true,
      ...
      show: { kind: 'graph', f: x => 3 * x + 1, title: 'ONE RULE THROUGH THREE POINTS', note: 'f(x)=3x+1', x0: 0, x1: 4.5, y0: 0, y1: 14, w: 280, h: 205,
        marks: [[1, 4], [2, 7], [3, 10]],
        caption: 'The simplest rule that fits. Infinitely many curves pass through the same three points, so the table alone cannot prove this is the right one.' } },
    { q: 'If f(2) = 9, write that fact as an ordered pair, as a sentence, and as a point on a graph.', level: 'Represent',
      ...
      show: { kind: 'frames', label: 'One fact, three ways',
        frames: [
          { kind: 'machine', rule: 'f', input: '2', output: '9', pick: 'as a machine', say: 'The input 2 produces the output 9.' },
          { kind: 'mapping', pairs: [['2', '9']], pick: 'as a set map', say: 'One arrow, from 2 to 9.' },
          { kind: 'graph', f: x => 4.5 * x, title: 'AS A POINT', note: '(2, 9)', x0: -1, x1: 4, y0: -2, y1: 12, w: 250, h: 190, marks: [[2, 9]], pick: 'as a point',
            say: 'Two across, nine up. The curve through it is a guess; the point is the fact.' }
        ] } },
  ],
