...
{ kind: 'evaluate', name: 'f', expr: 'sqrt(x)', f: x => Math.sqrt(x), at: [0, 1, 4, 9, 16] },
  ...
  practice: [
    { q: 'State the domain of q(x) = 1/(x − 3).', level: 'Calculate',
      ...
      show: { kind: 'numberline', from: -3, to: 8, spans: [{ a: -3, b: 3 }, { a: 3, b: 8 }], marks: [{ x: 3, open: true, label: 'refused' }],
        caption: 'A hollow endpoint is a removed point. The line falls into two stretches, which is why the notation needs two brackets.' } },
    { q: 'State the real domain and range of r(x) = sqrt(x + 2).', level: 'Calculate',
      a: 'Domain [−2, infinity); range [0, infinity).',
      show: { kind: 'graph', f: x => Math.sqrt(x + 2), title: 'WHERE IT STARTS', note: 'sqrt(x+2)', x0: -5, x1: 5, y0: -2, y1: 4, w: 275, h: 200, marks: [[-2, 0]],
        caption: 'Nothing is drawn to the left of −2, because nothing is allowed there. The curve begins at height 0 and only rises, which is the range read off the picture.' } },
    { q: 'The input is time from 0 to 60 seconds. Write that contextual domain in words and as an interval.', level: 'Represent',
      ...
      show: { kind: 'numberline', from: -20, to: 80, ticks: false,
        spans: [{ a: -20, b: 0, tone: 'out', label: 'before the start' }, { a: 0, b: 60, label: '[0, 60]' }, { a: 60, b: 80, tone: 'out', label: 'after the end' }],
        marks: [{ x: 0, label: '0 s' }, { x: 60, label: '60 s' }],
        caption: 'Both endpoints are filled, because both moments are included. Here the domain comes from the situation rather than the algebra.' } },
    { q: 'State the domain of f(x) = 1/(x + 5).', level: 'Calculate',
      a: 'All real x except −5, since x + 5 = 0 at x = −5.',
      show: { kind: 'numberline', from: -10, to: 4, spans: [{ a: -10, b: -5 }, { a: -5, b: 4 }], marks: [{ x: -5, open: true, label: 'refused' }],
        caption: 'The denominator vanishes where x + 5 = 0, which is at −5 rather than at 5. The condition belongs to the whole denominator.' } },
    { q: 'State the domain of f(x) = sqrt(5 − x).', level: 'Calculate',
      ...
      show: { kind: 'numberline', from: -3, to: 9, spans: [{ a: -3, b: 5, label: 'allowed' }, { a: 5, b: 9, tone: 'out', label: 'refused' }], marks: [{ x: 5, label: 'included' }],
        caption: 'The shading runs the other way here. Moving x across the inequality reverses it, so the domain is everything up to 5 rather than everything from it.' } },
    { q: 'State the domain of f(x) = x^2 + 4x − 7.', level: 'Calculate',
      ...
      show: { kind: 'numberline', from: -6, to: 6, spans: [{ a: -6, b: 6, label: 'every real number' }],
        caption: 'No denominator and no even root, so there is nothing to scan for and nothing is refused. A polynomial never restricts its own domain.' } },
    { q: 'Write "every real number except 0 and 2" in interval notation.', level: 'Represent',
      ...
      show: { kind: 'numberline', from: -5, to: 7, spans: [{ a: -5, b: 0 }, { a: 0, b: 2 }, { a: 2, b: 7 }],
        marks: [{ x: 0, open: true }, { x: 2, open: true }],
        caption: 'Two points removed leaves three stretches, so the notation needs three brackets. Count the pieces from the picture and the answer writes itself.' } },
    { q: 'What is the range of f(x) = x^2 + 3?', level: 'Calculate',
      ...
      show: { kind: 'graph', f: x => x * x + 3, title: 'NOTHING BELOW 3', note: 'x^2+3', x0: -4, x1: 4, y0: 0, y1: 12, w: 275, h: 205, marks: [[0, 3]],
        caption: 'Squaring never returns less than 0, so adding 3 never returns less than 3, and the marked low point is where 3 is actually reached.' } },
    { q: 'A square has side x. Write the domain of its area function in context, and say why it differs from the algebraic domain.', level: 'Recognise',
      ...
      show: { kind: 'numberline', from: -6, to: 6,
        spans: [{ a: -6, b: 0, tone: 'out', label: 'algebra allows, geometry does not' }, { a: 0, b: 6, label: 'a real side length' }],
        marks: [{ x: 0, open: true }],
        caption: 'x² accepts negatives quite happily. A side of length −3 is the part the situation refuses, which is the third cause of exclusion and the one algebra cannot see.' } },
    { q: 'State the domain of f(x) = sqrt(x)/(x − 4).', level: 'Calculate', hard: true,
      ...
      show: { kind: 'numberline', from: -3, to: 9, spans: [{ a: -3, b: 0, tone: 'out', label: 'root refuses' }, { a: 0, b: 4 }, { a: 4, b: 9 }],
        marks: [{ x: 0, label: 'kept' }, { x: 4, open: true, label: 'removed' }],
        caption: 'Two causes, two different marks. The root closes the left end and the denominator punches a hole, and both have to be checked.' } },
    { q: 'State the domain of f(x) = 1/sqrt(x − 1).', level: 'Calculate', hard: true,
      ...
      show: { kind: 'numberline', from: -3, to: 8, spans: [{ a: -3, b: 1, tone: 'out', label: 'refused' }, { a: 1, b: 8, label: 'allowed' }],
        marks: [{ x: 1, open: true, label: 'lost as well' }],
        caption: 'Compare with the domain of sqrt(x − 1), where 1 is included. Underneath a fraction the root may not be zero, so the endpoint goes hollow.' } },
    { q: 'Two functions have the same formula but domains [0, 3] and all real numbers. Are they the same function?', level: 'Recognise', hard: true,
      ...
      show: { kind: 'frames', label: 'Same formula, different functions',
        frames: [
          { kind: 'graph', f: x => (x >= 0 && x <= 3) ? x * x : NaN, title: 'DOMAIN [0, 3]', note: 'refuses 5', x0: -4, x1: 6, y0: -1, y1: 12, w: 265, h: 200, marks: [[0, 0], [3, 9]], pick: 'restricted',
            say: 'Only a piece of the curve exists, because only those inputs are allowed.' },
          { kind: 'graph', f: x => x * x, title: 'ALL REAL NUMBERS', note: 'accepts 5', x0: -4, x1: 6, y0: -1, y1: 12, w: 265, h: 200, marks: [[5, 25]], pick: 'unrestricted',
            say: 'The same formula, drawn everywhere. It answers at 5 and the other refuses, so they behave differently and are different functions.' }
        ] } },
  ],
